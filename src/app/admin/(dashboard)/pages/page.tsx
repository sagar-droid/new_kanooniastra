...
const PagesListPage = async ({ searchParams }: PagesListPageProps) => {
            ...
            <tbody>
              {items.map((item) => (
                <tr key={String(item._id)} className="border-t border-gray-100">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-gray-600 whitespace-nowrap">/{item.slug}</td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex gap-3">
                      ...
                      <DeleteButton
                        endpoint={`/api/admin/pages/${item._id}`}
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex gap-2">
          {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
            <Link
              key={pageNumber}
              href={{ pathname: "/admin/pages", query: { ...searchParams, page: pageNumber } }}
